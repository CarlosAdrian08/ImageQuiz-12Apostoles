...
let correctLetter = '';
let correctWord = '';
let letterIndex = -1; // Agregar índice de letra
...
function startGame() {
    ...
    apostles.forEach(apostle => {
        ...
        apostle.images.forEach((image, index) => {
            ...
            card.setAttribute('data-index', index); // Agregar índice a la carta
            ...
            card.onclick = () => openModal(image, apostle.name.charAt(index).toUpperCase(), apostle.id, index);
...
function openModal(image, letter, apostleId, index) {
    ...
    letterIndex = index; // Guardamos el índice de la letra
...
function checkAnswer() {
    ...
    if (userInput === correctWord) {
        resultMessage.textContent = '¡Correcto! Tienes +1 punto';

        // Cambiar la imagen de la carta a la letra correspondiente
        const selectedCard = document.querySelector(`.card[data-id="${currentCard}"][data-index="${letterIndex}"]`);
        ...
        resultMessage.textContent = 'Inténtalo de nuevo. Tienes -1 punto';
    }

    // Mostrar el modal con el resultado
    resultModal.style.display = 'flex';
    closeModal(); // Cerrar el modal de respuesta
}

function retry() {
    const resultModal = document.getElementById('resultModal');
    resultModal.style.display = 'none'; // Cerrar el modal de resultado
    document.getElementById('guessInput').value = ''; // Limpiar el input
}